...
$cat_id=2;
    ...
    $query = "SELECT * FROM emergancy_details where case_id= $caseID;"; 
    $result = $con->query($query); 
		...
		echo "data is available";//go to update form
			...
			// new form
		$result1 = mysqli_query($con,"SELECT * from emergancy_master where cat_id=$cat_id");
...
<td class="col-md-5"><?php echo $row['em_name'];?></td>
...
// insert in to database
	function insert_into_db($php_curd){
			foreach($php_curd as $key => $value){
		$k[] = $key;
		$v[] = "'".$value."'";
	}
	$k=implode(",", $k);
	...
	$conn= mysqli_connect("localhost","root","","emergency_crisis");
	$sql="INSERT INTO emergancy_details($k) VALUES($v)";
